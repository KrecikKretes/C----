...
void Koszty(int** g, int** koszt, int r) {
	int suma = 0;

	for (int i = 0; i < r; i++) {
		for (int j = 0; j < r; j++) {
			if (g[i][j] != i && koszt[i][j]!=10000) {
				suma++;
				cout << " Wiersz : " << i << "\n Kolumna : "<< j<<endl;
			}
		}
...
int main(){
	...
	*/
	...
	int** k = Graf(n);
	int** koszt = Graf(n);
	WczytajGraf("koszt.txt", k, n);
	WczytajGraf("koszt.txt", koszt, n);
	for (int i = 0; i < n; i++)
		for (int j = 0; j < n; j++)
			if (k[i][j] == 0) {
				k[i][j] = 10000;
	...
	cout << "\n\n koszty, ktore nie sa proste: \n"; Koszty(r,koszt, n); cout << "\n\n";
	//-----------------------------------------
	...
	dijkstra(k, n, pocz);
	cout << endl;
	cout << "\nwektor d po alg.: "; for (int i = 0; i < n; i++) cout << " " << d[i];
	cout << "\n wekt optPoprz po alg., A=0...F=5: "; for (int i = 0; i < n; i++) cout << " " << optPoprz[i];
	*/
}